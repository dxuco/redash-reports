...
game_product rows) and emits overview-data.json:
  ...
  months[ym] = {
      ...
      depAmount:    [$ per day]
      withdrawers:  [n per day]     distinct player_id with withdraw > 0
      wdAmount:     [$ per day]     withdrawn dollars, split by rail like deposits
...
def build_view(rows, exclude=()):
    ...
    wd_players = defaultdict(set)       # date -> {player}
    wd_amount = defaultdict(float)
    wd_rail_amount = defaultdict(lambda: defaultdict(float))  # date -> rail -> $
    wd_rail_players = defaultdict(lambda: defaultdict(set))
    ...
    for r in rows:
        ...
        # Withdrawals. Same shape as deposits and split by the same rail list --
        # `withdraw` never shares a row with `game_product`, `bet`, `ggr` or
        # `adjusted_ggr` (zero overlap in Aug 2026), but it always carries a
        # `blockchain`, so the crypto/fiat split is measured, not inferred.
        # Values arrive positive (dollars leaving the business); they are kept
        # positive so the chart reads the same way as the deposit chart beside
        # it, and the net figure does the subtracting.
        wd = num(r.get("withdraw"))
        if wd > 0:
            if pid:
                wd_players[d].add(pid)
                month_wd.add(pid)
            rl = rail_of(r.get("blockchain"))
            wd_rail_amount[d][rl] += wd
            if pid:
                wd_rail_players[d][rl].add(pid)
        wd_amount[d] += wd
    ...
    days = sorted(set(list(dep_players) + list(bet_players) + list(dep_amount) +
                      list(wd_players) + list(wd_amount)))
    ...
    cum_dep, cum_bet, cum_wd = [], [], []
    seen_d, seen_b, seen_w = set(), set(), set()
    ...
    # Withdrawal rails are listed separately: every withdraw row carries a
    # blockchain, so "unknown" is empty here even in months where deposits have
    # one. Filtering per chart keeps a dead legend chip off the withdrawal card.
    wd_rails = ["crypto", "fiat", "unknown"]
    wd_rails = [r for r in wd_rails if any(wd_rail_amount[d].get(r) for d in days)]
    ...
    return sets, {
        ...
        "wdRails": wd_rails,
        "wdRailAmt": {rl: [round(wd_rail_amount[d].get(rl, 0.0), 2) for d in days] for rl in wd_rails},
        "wdRailPlayers": {rl: [len(wd_rail_players[d].get(rl, ())) for d in days] for rl in wd_rails},
        "cumDep": cum_dep,
        "cumWd": cum_wd,
        "cumBet": cum_bet,
        "cumCat": cum_cat,
        ...
        "withdrawers": [len(wd_players[d]) for d in days],
        "wdAmount": [round(wd_amount[d], 2) for d in days],
...
def main():
    ...
    for f in files:
        ...
        w_ex, w_inc = sum(ex["wdAmount"]), sum(inc["wdAmount"])
        n = len(ex["days"]) or 1
        print("%s  %2d days  deposits/day  ex-whale $%-9s  incl $%-10s  whale is %3d%%"
              "   |  withdrawals/day  ex-whale $%-9s  incl $%-10s  whale is %3d%%"
              % (ym, n, format(round(d_ex / n), ","), format(round(d_inc / n), ","),
                 round((d_inc - d_ex) / d_inc * 100) if d_inc else 0,
                 format(round(w_ex / n), ","), format(round(w_inc / n), ","),
                 round((w_inc - w_ex) / w_inc * 100) if w_inc else 0))
    ...
    def range_distincts(view):
        ...
        for label, cap in (("full", None), ("mtd", mtd_days)):
            dep, wd, bet, cat = set(), set(), set(), defaultdict(set)
            for ym in order:
                s = per[ym]
                n = len(s["dep"]) if cap is None else cap
                for day_set in s["dep"][:n]:
                    dep |= day_set
                for day_set in s["wd"][:n]:
                    wd |= day_set
                ...
                for c, day_sets in s["cat"].items():
                    for day_set in day_sets[:n]:
                        cat[c] |= day_set
            out[label] = {"dep": len(dep), "wd": len(wd), "bet": len(bet),
    ...
    for v in ("ex", "inc"):
        print("range distinct (%s):  full  %s depositors / %s withdrawers / %s bettors    "
              "mtd  %s / %s / %s"
              % (v, format(rng[v]["full"]["dep"], ","), format(rng[v]["full"]["wd"], ","),
                 format(rng[v]["full"]["bet"], ","),
                 format(rng[v]["mtd"]["dep"], ","), format(rng[v]["mtd"]["wd"], ","),
                 format(rng[v]["mtd"]["bet"], ",")))
